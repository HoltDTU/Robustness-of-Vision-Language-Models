"""
Produktions-loop for VLM-graf-evaluering (02445).
=================================================
24 niveauer x 10 distinkte grafer = 240 grafer. temperature=0.0 (deterministisk).
3 isolerede spoergsmaal pr. graf (knuder/kanter/orientering).
...
ROBUSTHED MOD FORBINDELSESTAB:
  ...
  * En raekke skrives KUN, naar alle 3 svar er modtaget. Falder forbindelsen,
...
COL_EDGES = "n_edges_true"
COL_DIR   = "directed_true"

TASKS = {
    "blue":    {"prompt": "Hvor mange blå knuder (cirkler) er der i denne graf?",
    "red"
}
""""nodes":    {"prompt": "Hvor mange knuder (cirkler) er der i denne graf? "
                           "Svar kun med et tal.", "kind": "count", "pred_col": "pred_nodes"},
    "edges":    {"prompt": "Hvor mange kanter (streger) er der i denne graf? "
                           "Svar kun med et tal.", "kind": "count", "pred_col": "pred_edges"},
    "directed": {"prompt": "Er grafen rettet eller urettet (har kanterne pile)? "
                           "Svar kun med ét ord: rettet eller urettet.",
                 "kind": "directed", "pred_col": "pred_directed"}"""

PRED_FIELDS = ["image_id", "model", "pred_nodes", "pred_edges", "pred_directed"]
...
def parse_directed(text):
    if not text:
        return None
    t = text.lower()
    if "urettet" in t or "undirected" in t:
        return "urettet"
    if "rettet" in t or "directed" in t:
        return "rettet"
    return None
...
def clean_predictions():
    ...
    bad = df["pred_nodes"].isna() & df["pred_edges"].isna()
    if bad.any():
        ...
        for model in MODELS:
            ...
            try:
                for task_name, task in TASKS.items():
                    ...
                    pred_row[task["pred_col"]] = (
                        parse_count(raw) if task["kind"] == "count" else parse_directed(raw))
                    raw_rows.append({
...
def quick_accuracy(manifest):
    ...
    for model, g in m.groupby("model"):
        print(f"  {model:30s} | knuder={(g['pred_nodes']==g[COL_NODES]).mean():.0%} | "
              f"kanter={(g['pred_edges']==g[COL_EDGES]).mean():.0%} | "
              f"orientering={(g['pred_directed']==g[COL_DIR]).mean():.0%}")
